...
const entries = computed(() => {
  ...
  return [...menuVisibilityKeys].flatMap(key => settings.has(key) ? [settings.get(key)!] : [])
})
</script>
...
<template>
  <section
    ...
    :id="id"
    data-menu-visibility-group
    class="min-w-0 space-y-5"
    :aria-labelledby="`${id}-heading`"
  >
    <header class="space-y-2">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Menu visibility</h3>
      <p class="max-w-3xl text-sm leading-relaxed text-muted-foreground">Choose when and where MCCC menus appear.</p>
    </header>
