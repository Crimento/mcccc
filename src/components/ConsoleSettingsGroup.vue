...
import { consoleBlocks } from '@/lib/console-settings'
...
const blocks = computed(() => {
  ...
  return consoleBlocks.map(block => ({
    ...block,
...
<template>
  <section
    ...
    :id="id"
    data-console-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading !== false ? `${id}-heading` : undefined"
    :aria-label="showHeading === false ? 'Console Command Settings' : undefined"
  >
    <header v-if="showHeading !== false" class="space-y-2">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Console Command Settings</h3>
      <p class="max-w-3xl text-sm leading-relaxed text-muted-foreground">Configure visual effects, general cheats, and BuildBuy mode cheats.</p>
    </header>
