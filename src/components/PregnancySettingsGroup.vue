...
import type { PregnancySettingsGroup as PregnancyGroupDefinition } from '@/lib/pregnancy-groups'
import { Button } from '@/components/ui/button'
import SettingControl from './SettingControl.vue'

const props = withDefaults(defineProps<{
  group: PregnancyGroupDefinition
...
function isTraitPairSetting(key: string): boolean {
  return key === 'Marriage_RequiredTraitsList' || key === 'Marriage_ConflictTraitsList'
}
</script>
...
<template>
  <section
    ...
    :id="id"
    :data-pregnancy-settings-group="group.id"
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading ? `${id}-heading` : undefined"
    :aria-label="showHeading ? undefined : group.label"
  >
    ...
    >
      ...
      <div
        class="grid min-w-0 gap-x-8 gap-y-6 md:grid-cols-2"
        :class="block.columns === 3 ? 'lg:grid-cols-3' : undefined"
      >
        <article
          ...
          :class="isTraitPairSetting(setting.key) ? 'md:col-span-2' : undefined"
        >
