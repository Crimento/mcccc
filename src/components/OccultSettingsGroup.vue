...
import { occultSettingBlocks } from '@/lib/occult-groups'
...
const blocks = computed(() => {
  ...
  return occultSettingBlocks.map(block => ({
    ...block,
...
<template>
  <section
    ...
    :id="id"
    data-occult-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading ? `${id}-heading` : undefined"
    :aria-label="showHeading ? undefined : 'Occult settings'"
  >
    <header v-if="showHeading">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Occult settings</h3>
    </header>
    ...
    >
      ...
      <div class="grid min-w-0 gap-x-8 gap-y-6 md:grid-cols-2">
        <article
          ...
          :class="block.id === 'shared-aging' ? 'md:col-span-2' : ''"
        >
          ...
          <SettingControl
            ...
            :species="setting.occultAging ? selectedSpecies : undefined"
