...
import { autoRelationshipBlocks } from '@/lib/relationship-settings'
...
const blocks = computed(() => {
  ...
  return autoRelationshipBlocks.map(block => ({
    ...block,
...
<template>
  <section
    ...
    :id="id"
    data-auto-relationship-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading !== false ? `${id}-heading` : undefined"
    :aria-label="showHeading === false ? 'Auto-Relationship Settings' : undefined"
  >
    <h3 v-if="showHeading !== false" :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Auto-Relationship Settings</h3>

    <section
      v-for="block in blocks"
      v-show="block.settings.some(setting => visibleKeys.has(setting.key))"
      :key="block.id"
      :data-auto-relationship-block="block.id"
      :aria-labelledby="`${id}-${block.id}-heading`"
      class="min-w-0 space-y-5 rounded-xl bg-muted/15 p-4 sm:p-5"
    >
      <h4 :id="`${id}-${block.id}-heading`" class="text-sm font-semibold">{{ block.title }}</h4>
