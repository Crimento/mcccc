...
import { clubBlocks } from '@/lib/club-settings'
...
<template>
  <section
    ...
    :id="id"
    data-club-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading !== false ? `${id}-heading` : undefined"
    :aria-label="showHeading === false ? 'Clubs' : undefined"
  >
    <header v-if="showHeading !== false">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Clubs</h3>
    </header>
    ...
    >
      ...
      <div class="grid min-w-0 gap-x-8 gap-y-6 md:grid-cols-2" :class="block.id === 'membership' ? 'xl:grid-cols-3' : ''">
        <article
