...
import { tunerSettingBlocks } from '@/lib/tuner-settings'
...
const blocks = computed(() => {
  ...
  return tunerSettingBlocks.map(block => ({
    ...block,
...
<template>
  <section
    ...
    :id="id"
    data-tuner-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading ? `${id}-heading` : undefined"
    :aria-label="showHeading ? undefined : 'Tuner settings'"
  >
    <header v-if="showHeading">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Tuner settings</h3>
    </header>
