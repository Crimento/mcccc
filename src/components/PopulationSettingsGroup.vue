...
import { populationSettingBlocks } from '@/lib/population-settings'
...
const blocks = computed(() => {
  ...
  return populationSettingBlocks.map(block => ({
    ...block,
...
<template>
  <section
    ...
    :id="id"
    data-population-settings-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading ? `${id}-heading` : undefined"
    :aria-label="showHeading ? undefined : 'Population settings'"
  >
    <header v-if="showHeading">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">Population settings</h3>
    </header>
    ...
    <section
      ...
      :key="block.id"
      :data-population-block="block.id"
