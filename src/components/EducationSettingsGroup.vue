...
import { educationBlocks } from '@/lib/career-settings'
...
<template>
  <section
    ...
    :id="id"
    data-education-group
    class="min-w-0 space-y-6"
    :aria-labelledby="showHeading !== false ? `${id}-heading` : undefined"
    :aria-label="showHeading === false ? 'School and university' : undefined"
  >
    <header v-if="showHeading !== false">
      <h3 :id="`${id}-heading`" class="text-lg font-semibold tracking-tight">School and university</h3>
    </header>
    ...
    >
      ...
      <div class="grid min-w-0 gap-x-8 gap-y-6 md:grid-cols-2" :class="block.id === 'university' ? 'xl:grid-cols-3' : ''">
        <article
          ...
          :class="setting.key === 'Career_Homework_Speed' ? 'md:col-span-2' : ''"
        >
